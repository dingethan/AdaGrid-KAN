"""
PlateauGridExpander: 均匀 Grid 自动扩展调度器
==============================================
模仿 torch.optim.lr_scheduler.ReduceLROnPlateau 的 API 风格。

核心策略（与 visualization/index.html 一致）:
    Loss Plateau 检测
        ↓
    对所有 AdaptiveKANLayer 执行 expand_grid（均匀翻倍）
        ↓
...
class EcoGrowScheduler:
    ...
    def _find_kan_layers(self, model: nn.Module) -> List[AdaptiveKANLayer]:
        layers = [
            module for module in model.modules()
            if isinstance(module, AdaptiveKANLayer)
        ]
        if not layers:
            raise ValueError("No AdaptiveKANLayer found in the model.")
        return layers
    ...
    def _expand_all_layers(self, new_grid: int) -> None:
        for layer in self._kan_layers:
            if layer.grid_size < new_grid:
                layer.expand_grid(new_grid)
    ...
    def _start_trial_growth(self, train_loss: float, val_loss: float, epoch: int) -> EcoGrowResult:
        ...
        self._expand_all_layers(new_grid)
...
class PlateauGridExpander:
    """
    均匀 Grid 自动扩展调度器。

    当监控的 loss 在 patience 个 epoch 内没有改善时：
    对每个未达上限的 AdaptiveKANLayer 调用 expand_grid(new_G)，
    其中 new_G = min(current_G * 2, max_grid)。

    参数:
        model:          包含 AdaptiveKANLayer 的 nn.Module
        patience:       容忍停滞的 epoch 数（默认 10）
        max_grid:       Grid 区间数硬上限（默认 64）
        verbose:        是否打印翻倍日志
    ...
    使用示例:
        scheduler = PlateauGridExpander(model, patience=10, max_grid=64)
        for epoch in range(num_epochs):
            train_one_epoch(...)
            expanded, events = scheduler.step(loss.item(), epoch=epoch)
            if expanded:
            ...
            for layer in self._kan_layers:
                ...
                if self.verbose:
                    ...
                    print(
                        ...
                        f"Uniform expand_grid {old_g} -> {new_g}."
                    )
