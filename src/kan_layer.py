"""
DynamicKANLayer: 支持自适应网格细化的 KAN 层
================================================
核心创新：初始使用稀疏均匀 Grid（如 G=3），当训练遇到 plateau 时，
通过 refine_grid() 均匀翻倍扩展 Grid，并用 F.interpolate 平滑继承控制点。

refine_grid:    ★ 主方案 — 全局均匀翻倍 + F.interpolate 重采样（与浏览器 demo 一致）
insert_knot:    可选 — Boehm 局部插入，grid 变为非均匀（高级用法，保留接口）

B-Spline 基底函数采用 Cox-de Boor 递推公式实现，
支持任意阶数（默认 k=3，即三次 B-Spline）。
"""
...
class DynamicKANLayer(nn.Module):
    """
    动态网格 KAN 层，用于 1D 回归任务。
    ...
    def __init__(self, in_dim=1, out_dim=1, grid_size=3, spline_order=3,
                 grid_range=(-1.0, 1.0)):
        super().__init__()
        self.in_dim = in_dim
    ...
    def _compute_bspline_basis(self, x, knots):
        """
    ...
    def forward(self, x):
        ...
        # B-Spline 样条部分
        spline_out = torch.zeros(batch_size, self.out_dim, device=device)
        ...
        for d in range(self.in_dim):
            ...
            knots = self._extend_knots(grid)
            basis = self._compute_bspline_basis(x_d_clamped, knots)

            cp_d = self.control_points[:, d, :]  # (out_dim, G+k)
    ...
    def refine_grid(self, new_grid_size):
        """
        全局均匀加密（主方案，与 ExtendGridOnPlateau / 浏览器 demo 一致）。
        ...
        new_cp = new_cp_flat.squeeze(1).reshape(self.out_dim, self.in_dim, new_n_basis)
    ...
    def insert_knot(self, x_bar):
        """
        ★ 局部自适应加密（新方法）：Boehm's Knot Insertion ★
        ...
        算法来源: Piegl & Tiller, "The NURBS Book", 2nd ed., P5.1
        ...
        old_knots = self._extend_knots(old_grid)  # (G + 2k,)
        ...
        # ---- 5. Boehm 算法：计算新控制点 ----
        # 参考 NURBS Book P5.1: α_j = (u - u_j) / (u_{j+p} - u_j)
        #   新控制点 Q_j:
        #     j ≤ i-p     → Q_j = P_j           （不变）
        #     i-p < j ≤ i → Q_j = (1-α_j)·P_{j-1} + α_j·P_j  （混合）
        #     j > i       → Q_j = P_{j-1}         （右移）
        old_cp = self.control_points.data  # (out_dim, in_dim, n_cp)
        n_cp_new = n_cp + 1
        new_cp = torch.zeros(self.out_dim, self.in_dim, n_cp_new,
        ...
        blend_end = min(i, n_cp)  # j 的上界是 i（含），并且 j 索引旧控制点时 ≤ n_cp-1
        for j in range(blend_start, blend_end + 1):
            ...
            # 注意：j 索引新控制点 Q_j，旧控制点 P_{j-1} 和 P_j 均在界内
    ...
    def _boehm_transform_1d(tensor_3d, i, p, old_knots, x_bar):
        ...
        参数:
            tensor_3d: (out_dim, in_dim, n_cp)  需要变换的张量
            i, p, old_knots, x_bar: 与 insert_knot 相同的 Boehm 参数
        返回:
            (out_dim, in_dim, n_cp + 1)
        """
    ...
    def compute_interval_residuals(self, x, y_true):
        ...
        参数:
            x: (batch_size, in_dim)  输入张量
            y_true: (batch_size, out_dim)  目标值
