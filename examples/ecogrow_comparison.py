...
def train_fixed_kan(
    ...
    model = AdaptiveKANLayer(
        input_dim=1, output_dim=1, grid_size=args.fixed_grid,
...
def train_adagrid(
    ...
    torch.manual_seed(seed)
    model = AdaptiveKANLayer(
        input_dim=1, output_dim=1, grid_size=args.initial_grid,
        spline_order=3, grid_range=(-1.0, 1.0),
    ).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        expanded, expand_events = scheduler.step(loss.item(), epoch=epoch)
        if expanded:
