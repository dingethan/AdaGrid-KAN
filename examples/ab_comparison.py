#!/usr/bin/env python3
"""
AdaGrid-KAN A/B 对比实验（均匀 Grid 自动扩展 vs 传统固定 Grid）
================================================================
与 visualization/index.html 一致：
  橙: AdaGrid -- G=3 起步，plateau 均匀翻倍 -> G=64
...
def train_adagrid(seed=42):
    ...
    loss_history, grid_history, expand_events = [], [], []
    ...
    return model, loss_history, grid_history, expand_events, y_pred, final_mse
...
def train_traditional_fixed(seed, fixed_grid):
    ...
    torch.manual_seed(seed)
    model = AdaptiveKANLayer(
        input_dim=1, output_dim=1, grid_size=fixed_grid,
        spline_order=3, grid_range=(-1., 1.)
    ).to(DEVICE)
    optimizer = rebuild_optimizer(model)
...
# ==========================================================================
print("\n" + "=" * 60)
print("训练 AdaGrid-KAN（均匀自动翻倍 G=3->64）...")
print("=" * 60)
