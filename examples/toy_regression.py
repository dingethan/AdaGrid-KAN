#!/usr/bin/env python3
"""
AdaGrid-KAN 1D 回归演示（均匀 Grid 自动扩展）
=============================================
目标函数: y = sin(5x) + 0.5 * cos(15x)

策略（与 visualization/index.html 一致）:
  G=3 起步 -> loss plateau -> 均匀翻倍 -> 最长至 G=64
"""
...
from src.grid_scheduler import PlateauGridExpander
...
model = AdaptiveKANLayer(
    input_dim=1, output_dim=1, grid_size=INITIAL_GRID,
    spline_order=3, grid_range=(-1.0, 1.0),
).to(DEVICE)

scheduler = PlateauGridExpander(
...
for epoch in range(1, NUM_EPOCHS + 1):
    ...
    expanded, events = scheduler.step(loss_val, epoch=epoch)
    if expanded:
        for ep, old_g, new_g in events:
            expand_epochs.append(ep)
...
print("\n" + "=" * 60)
print(f"训练完成！Loss={loss_history[-1]:.6f}, Grid={model.grid_size}, 翻倍 {len(expand_epochs)} 次")
print("=" * 60)
...
if expand_epochs:
    ax1.axvline(x=expand_epochs[0], color='#ff7a1a', linestyle='--', linewidth=1.0,
                alpha=0.6, label=f'Grid double ({len(expand_epochs)}x)')
...
ax3.set_title('Grid Auto-Grow (3->6->12->24->48->64)')
