...
def _make_model(grid_size: int = 3, seed: int = 0) -> AdaptiveKANLayer:
    torch.manual_seed(seed)
    return AdaptiveKANLayer(input_dim=1, output_dim=1, grid_size=grid_size, spline_order=3)
...
class TestEcoGrowGrowthStarted:
    def test_growth_started_increases_grid(self):
        ...
        scheduler = EcoGrowScheduler(
            model, patience=3, max_grid=64, trial_epochs=5,
            verbose=False,
        )
...
class TestEcoGrowMaxGrid:
    def test_grid_never_exceeds_max_grid(self):
        ...
        scheduler = EcoGrowScheduler(
            model, patience=2, max_grid=64, trial_epochs=2,
        ...
        for epoch in range(30):
            ...
            model = result.model
            assert model.grid_size <= 64

        # 40 * 2 = 80 capped to 64
        assert model.grid_size == 64
...
class TestEcoGrowStateDict:
    ...
    def test_state_dict_roundtrip_normal_state(self):
        ...
        model = _make_model(3)
        scheduler = EcoGrowScheduler(
            model, patience=3, max_grid=64, trial_epochs=3, verbose=False,
        )
        # 驱动几步
        ...
        scheduler2 = EcoGrowScheduler(
            model2, patience=3, max_grid=64, trial_epochs=3, verbose=False,
        )
    ...
    def test_state_dict_preserves_rejected_growths(self):
        ...
        scheduler2 = EcoGrowScheduler(
            model2, patience=2, max_grid=64, trial_epochs=2,
...
class TestPlateauGridExpanderRepr:
    def test_repr_contains_class_name(self):
        model = _make_model(3)
        scheduler = __import__(
            "src.grid_scheduler", fromlist=["PlateauGridExpander"]
        ).PlateauGridExpander(model, verbose=False)
        r = repr(scheduler)
        assert "PlateauGridExpander" in r
