"""
DynamicKANLayer 单元测试
========================
覆盖前向传播、B-Spline 基函数、网格细化、Boehm 节点插入与区间残差分析。
"""
...
def _make_layer(in_dim=1, out_dim=1, grid_size=3, spline_order=3, seed=0):
...
def _rand_input(batch=16, in_dim=1, seed=0):
    torch.manual_seed(seed)
    return torch.rand(batch, in_dim) * 2 - 1  # uniform in [-1, 1]
...
class TestInit:
    ...
    def test_control_points_shape(self):
        layer = _make_layer(in_dim=2, out_dim=3, grid_size=4, spline_order=3)
        assert layer.control_points.shape == (3, 2, 7)  # (out, in, G+k)

    def test_base_weight_shape(self):
        layer = _make_layer(in_dim=2, out_dim=3)
    ...
    def test_extra_repr(self):
        layer = _make_layer(in_dim=1, out_dim=1, grid_size=3)
        s = layer.extra_repr()
...
class TestForward:
    def test_output_shape_1d(self):
        layer = _make_layer(in_dim=1, out_dim=1)
        x = _rand_input(batch=8, in_dim=1)
        y = layer(x)
        assert y.shape == (8, 1)

    def test_output_shape_multi(self):
        layer = _make_layer(in_dim=3, out_dim=2, grid_size=5)
        x = _rand_input(batch=10, in_dim=3)
        y = layer(x)
        assert y.shape == (10, 2)
...
class TestBSplineBasis:
    def test_basis_nonnegative(self):
        ...
        knots = layer._extend_knots(layer.grid)
        basis = layer._compute_bspline_basis(x, knots)
        assert (basis >= -1e-6).all(), "B-Spline 基函数值应为非负"
    ...
    def test_basis_partition_of_unity(self):
        ...
        assert torch.allclose(sums, torch.ones_like(sums), atol=1e-5), \
            f"基函数之和偏离 1，最大偏差 {(sums - 1).abs().max().item():.2e}"
    ...
    def test_extend_knots_length(self):
        layer = _make_layer(grid_size=5, spline_order=3)
        knots = layer._extend_knots(layer.grid)
...
class TestRefineGrid:
    ...
    def test_control_points_shape_updated(self):
        layer = _make_layer(in_dim=1, out_dim=1, grid_size=3, spline_order=3)
        layer.refine_grid(6)
        assert layer.control_points.shape == (1, 1, 9)   # (out, in, G+k) = (1,1,6+3)

    def test_output_still_finite_after_refine(self):
        layer = _make_layer(grid_size=3)
        layer.refine_grid(6)
    ...
    def test_refine_raises_on_smaller_size(self):
        layer = _make_layer(grid_size=6)
        with pytest.raises(ValueError):
            layer.refine_grid(3)

    def test_refine_multiple_times(self):
        layer = _make_layer(grid_size=3)
        layer.refine_grid(6)
        layer.refine_grid(12)
        assert layer.grid_size == 12
        x = _rand_input(batch=8)
        assert torch.isfinite(layer(x)).all()

    def test_refine_preserves_gradient(self):
        layer = _make_layer(grid_size=3)
        layer.refine_grid(6)
        x = _rand_input(batch=8)
        y = layer(x)
...
class TestInsertKnot:
    ...
    def test_curve_shape_preserved(self):
        ...
        assert torch.allclose(y_before, y_after, atol=1e-5), \
            f"Boehm 插入后曲线发生偏移，最大误差 {(y_before - y_after).abs().max().item():.2e}"
...
class TestNParams:
    ...
    def test_n_params_increases_after_refine(self):
        layer = _make_layer(grid_size=3)
