...
### Adaptive Grid Expansion with Cost-Aware Reversible Growth for KAN
...
scheduler = PlateauGridExpander(model, patience=8, max_grid=50)
...
[dashboard.html](dashboard.html) is an interactive experiment dashboard summarizing all results with Chart.js.
...
│   ├── kan_layer.py              # AdaptiveKANLayer
│   └── grid_scheduler.py        # PlateauGridExpander + EcoGrowScheduler
├── examples/
│   ├── toy_regression.py          # 1D toy regression demo
│   ├── ab_comparison.py          # AdaGrid vs Fixed Grid A/B test
│   ├── ecogrow_comparison.py     # EcoGrow three-way comparison
│   ├── real_dataset_regression.py # Multi-dimensional / synthetic dataset experiments
│   ├── ablation_study.py         # Noise robustness & hyperparameter sensitivity
│   ├── classification_experiment.py # Classification task experiments
│   └── generate_dashboard.py     # Generate interactive HTML dashboard
├── tests/
│   ├── test_kan_layer.py         # AdaptiveKANLayer unit tests
...
├── dashboard.html                # Interactive experiment dashboard
├── .github/workflows/
...
- Experiments cover **1D regression**, **multi-dimensional synthetic regression**, and **classification** tasks.
- Real-dataset experiments use synthetic benchmarks (Friedman #1, custom multi-D functions); UCI datasets require `scikit-learn`.
...
### Multi-dimensional / synthetic dataset regression

```bash
python examples/real_dataset_regression.py --datasets synthetic_4d synthetic_8d friedman
```

Compares Fixed KAN, AdaGrid-KAN, EcoGrow-KAN and MLP on multi-dimensional regression tasks with normalized inputs.

### Classification

```bash
python examples/classification_experiment.py
```

Uses `KANClassifier` (KAN feature transform + linear head) on moons, circles, and higher-dimensional classification benchmarks.

### Ablation study

```bash
# Noise robustness (5 seeds x 7 noise levels)
python examples/ablation_study.py --seeds 42 43 44 45 46

# Hyperparameter sensitivity only
python examples/ablation_study.py --skip-noise --seeds 42 43 44 45 46
```
